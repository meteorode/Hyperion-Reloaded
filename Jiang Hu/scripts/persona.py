...
def split_sent(sent):   # Use pos_ == PUNCT to split sent to several span
    ...
    for i in range(sent_len):
        if sent[i].pos_ == 'PUNCT':
            spans.append(sent[punch_pointer:i])
            punch_pointer = i
    return spans
...
def test(): 
    ...
    context = []
    name = '郭靖'
    test_doc = nlp("郭靖沉吟片刻，说道：“大汗武功之盛，古来无人能及。只是大汗一人威风赫赫，天下却不知积了多少白骨，流了多少孤儿寡妇之泪。”")
    test_sents = list(test_doc.sents)
